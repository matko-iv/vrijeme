...
export class CheckpointLayer {
  ...
  private drawBarriers() {
    ...
    for (const gate of this.gates) {
      ...
      const bx = x+nx*1.6*zoom, by = y+ny*1.6*zoom;
      ...
      gfx.circle(bx,by,zoom*.3).fill(0x51575c);
      gfx.moveTo(bx,by).lineTo(ex,ey).stroke({color:0xe7e4da,width:zoom*.32});
      if (closed) for (let i=0;i<3;i++) {
        const t=i/3, t2=t+.16;
        gfx.moveTo(bx+(ex-bx)*t,by+(ey-by)*t).lineTo(bx+(ex-bx)*t2,by+(ey-by)*t2).stroke({color:0xaf5148,width:zoom*.32});
      }
    }
  }
