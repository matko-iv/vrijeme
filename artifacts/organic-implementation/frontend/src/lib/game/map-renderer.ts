...
import { areTerrainTexturesLoaded, installationFrameMs, installationFrames, getTextureByUrl } from './tile-textures';
...
export class MapRenderer {
  ...
  constructor(app: Application) {
    ...
    this.worldContainer.addChild(this.organic.container);
  ...
  setSeason(season: Season) {
    ...
    if (changed) { this.dirty = true; this.organic.dirty = true; }
  }
  ...
  loadTiles(
    ...
    this.organic.dirty = true;
    this.transport = new TransportNetwork(this.tiles, this.networkEdges);
    this.cities.setWorld(this.tiles, this.transport, this.lakeHexes);
  ...
  private async preloadTerrainTileTextures() {
    ...
    try {
      ...
      // Keep the map interactive with flat ground and vector cities if imagery fails.
      console.warn('[map-renderer] Organic imagery unavailable; using flat terrain', error);
  ...
  private redraw() {
    ...
    const drawResources = this._showIcons && z > 0.6 && (this.mode === 'terrain' || this.mode === 'economic');
    const drawCityMarkers = this._showIcons && z > 0.3;
    ...
    // Continuous terrain and cached architectural surfaces share world coordinates.
    // Political mode adds its nation tint above both layers.
    const useTextures = this._showTextures
      && (this.mode === 'terrain' || this.mode === 'political');
    const useOrganic = useTextures && this.organic.ready;
    ...
    this.cities.draw(vis, visCount, this.transport, z, useTextures, this.lakeHexes, w, h);
    this.tileContainer.removeChildren();
    this.tileContainer.visible = false;
    this.animatedTiles.length = 0;
    this.baseGfx.clear();
    this.baseGfx.visible = useTextures && !useOrganic;
    ...
    for (let vi = 0; vi < visCount; vi++) {
      ...
      if (useTextures) {
        if (!useOrganic) {
          const key = hexKey(tile.q, tile.r);
          const terrain = this.lakeHexes.has(key) ? 0 : this.season === 'winter' && this.snowHexes.has(key) ? 5 : tile.terrain;
          this.baseGfx.poly(corners()).fill({ color: TERRAIN_TYPES[terrain]?.color ?? 0x475344 });
        }
        ...
        if (drawBorders && (!useOrganic || this._showBorders)) borderPolys.push(corners());
      } else {
  ...
  private drawResourceDots(
    ...
    const organic = this._showTextures && (this.mode === 'terrain' || this.mode === 'political');
    const dotR = organic ? Math.min(5, Math.max(1.5, sz * .15)) : Math.max(1.5, sz * .15);
    ...
    const scale = (organic ? Math.min(14, sz * .42) : sz * .42) / RESOURCE_LABEL_BASE_FONT;
    let used = 0;
    for (const d of dots) {
      ...
      for (const kind of ['road', 'rail'] as const) {
        ...
        for (const v of [-deck * 0.5, deck * 0.5]) {
          gfx.moveTo(...at(-half, v)).lineTo(...at(half, v))
            .stroke({ color: 0xc8c5ba, width: metres(.18) * z, alpha: 0.7 });
        }
        ...
        if (z > 1.1) for (const u of [-half * 0.55, half * 0.55]) {
          gfx.moveTo(...at(u, -deck * 0.5)).lineTo(...at(u, deck * 0.5))
            .stroke({ color: 0x3f3a34, width: metres(.28) * z, alpha: 0.55 });
        }
      }
    }
  }
  ...
  private drawNetworkLines(gfx: Graphics, offsetX: number, offsetY: number, z: number) {
    ...
    const roadWidth = ROAD_WIDTH * z;
    const railWidth = metres(.09) * z;
    const tieLength = metres(2.6) / 2 * z;
    ...
    for (const pass of [0, 1]) for (const e of roadEdges) {
      ...
      const width = dirt ? DIRT_WIDTH * z : roadWidth;
      if (pass === 0) {
        trace(pts);
        gfx.stroke({ color: dirt ? 0x8d7955 : 0x989184, width: width + ROAD_SHOULDER * 2 * z, alpha: dirt ? .6 : .95, cap: 'round', join: 'round' });
        continue;
      }
      trace(pts);
      gfx.stroke({ color: dirt ? 0xb99a6a : busy ? 0x515755 : 0x60635d, width, alpha: .98, cap: 'round', join: 'round' });
      if (dirt && z > 1.1) for (const lane of [-DIRT_WIDTH/4, DIRT_WIDTH/4]) {
        trace(path.lengths.map(d => {
          const p = samplePath(path, d, false, lane);
          return { x: p.x * z + offsetX, y: p.y * z + offsetY };
        }));
        gfx.stroke({ color: 0x887148, width: z * metres(.28), alpha: .65 });
      }
      ...
      if (z > 3 && !dirt) {
        ...
        for (let d = start + 1; d < end - 1; d += metres(9)) {
          const a = samplePath(path, d), b = samplePath(path, Math.min(d + metres(3), path.length - 1));
          gfx.moveTo(a.x * z + offsetX, a.y * z + offsetY).lineTo(b.x * z + offsetX, b.y * z + offsetY);
        }
        gfx.stroke({ color: 0xd9d7c7, width: MARKING_WIDTH * z, alpha: .8 });
      }
    }
    ...
    for (const [key, pos] of roadHexScreenPos) {
      if (pos.count >= 3) {
        ...
        const arms = paths.map(path => ({ path, p: samplePath(path, ROAD_WIDTH * .8, path.b === key) }));
        const outline = arms.flatMap(({ path, p }) => [-1, 1].map(side => {
          const half = (path.surface === 'dirt' ? DIRT_WIDTH : ROAD_WIDTH) / 2;
        ...
        gfx.poly(outline.flatMap(p => [p.x * z + offsetX, p.y * z + offsetY])).fill(dirt ? 0xb99a6a : busy ? 0x515755 : 0x60635d);
        if (!dirt && z > 3) {
          ...
          arms.forEach(({ path }, i) => {
            ...
            const p = samplePath(path, ROAD_WIDTH * 1.1, path.b === key), nx = -Math.sin(p.angle), ny = Math.cos(p.angle);
            gfx.moveTo((p.x - nx * ROAD_WIDTH / 2) * z + offsetX, (p.y - ny * ROAD_WIDTH / 2) * z + offsetY)
              .lineTo((p.x + nx * ROAD_WIDTH / 2) * z + offsetX, (p.y + ny * ROAD_WIDTH / 2) * z + offsetY)
              .stroke({ color: 0xeee5ce, width: z * MARKING_WIDTH * 2 });
          });
        }
      }
    }
    ...
    for (const e of railEdges) {
      ...
      for (let d = 1.5; d < path.length; d += metres(.65)) {
      ...
      gfx.stroke({ color: 0x4c3928, width: metres(.20) * z, alpha: .95 });
      for (const side of [-1, 1]) {
        const points = path.lengths.map(d => {
          const p = samplePath(path, d, false, side * RAIL_GAUGE / 2, false);
          return { x: p.x * z + offsetX, y: p.y * z + offsetY };
        });
        trace(points);
    ...
    const naturalPower = this._showTextures && (this.mode === 'terrain' || this.mode === 'political');
    for (const e of pwEdges) {
      ...
      if (e.hv) {
        ...
        const hvGap = naturalPower ? metres(2) * z : Math.max(1.5, z * 2.5);
        const hvColor = naturalPower ? 0x53554c : 0xf59e0b;
        const hvLineW = naturalPower ? metres(.07) * z : Math.max(0.6, z * 0.8);
        const sagAmt = naturalPower ? metres(.7) * z : Math.max(1.5, z * 3);
        ...
        const pwLineW = naturalPower ? metres(.045) * z : Math.max(0.4, z * 0.5);
        const sagAmt = naturalPower ? metres(.5) * z : Math.max(1, z * 2);
    ...
    for (const [hexId, pos] of pwHexPos) {
      ...
      if (naturalPower) {
        const arm = metres(pos.hv ? 2.5 : 1.2) * z;
        gfx.moveTo(pos.x - arm, pos.y).lineTo(pos.x + arm, pos.y)
          .moveTo(pos.x, pos.y - arm * .6).lineTo(pos.x, pos.y + arm * .6)
          .stroke({ color: 0x8a8b79, width: metres(.14) * z, alpha: .85 });
        continue;
      }
