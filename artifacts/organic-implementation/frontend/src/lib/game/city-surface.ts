...
export function citySurface(plan: CityPlan, detail: number): Container {
    ...
    for (const b of plan.roofs) {
        ...
        c.shadowOffsetX = (.10 + (b.storeys ?? 2) * .09) * density; c.shadowOffsetY = (.12 + (b.storeys ?? 2) * .13) * density;
    ...
    for (const b of plan.roofs) {
        ...
        const wall = .035 + (b.storeys ?? 2) * .035;
        const kind = b.kind ?? 'hip';
        // Villas have an L-shaped footprint; this clip also shapes the roof eaves.
        if (kind === 'villa') {
            c.beginPath(); c.moveTo(-w,-h); c.lineTo(w*.15,-h); c.lineTo(w*.15,-h*.1); c.lineTo(w,-h*.1); c.lineTo(w,h); c.lineTo(-w,h); c.closePath(); c.clip();
        }
        const stone = ['#c4b99c', '#c6c0aa', '#aca998', '#d0bfa0'][b.tone];
        c.fillStyle = '#5e5d4b'; c.fillRect(-w + .035, -h + wall + .02, 2 * w, 2 * h);
        c.fillStyle = stone; c.fillRect(-w, -h + wall, 2 * w, 2 * h);
        // Small facade openings are visible on the sunlit wall below the eave.
        c.fillStyle = '#303e36';
        for (let x = -w + .13; x < w - .06; x += .23) c.fillRect(x, h + .014, .07, wall * .65);
        const hue = [26, 29, 42, 24][b.tone], sat = [27, 18, 7, 31][b.tone];
        const light = [47, 43, 49, 54][b.tone] + random() * 6 - 3;
        const hip = kind === 'gable' || kind === 'terrace' || kind === 'barn' ? 0 : Math.min(w * .8, h * .36), ridge = h - hip;
        const facets = [
        ...
        if (kind !== 'apartment' && kind !== 'warehouse') for (const f of facets) {
        ...
        if (kind === 'apartment' || kind === 'warehouse') {
            c.fillStyle = kind === 'apartment' ? '#a9a797' : '#929a96'; c.fillRect(-w,-h,w*2,h*2);
            c.fillStyle = '#5b615746'; c.fillRect(-w+.05,-h+.05,w*2-.1,h*2-.1);
            c.strokeStyle = '#ded7be'; c.lineWidth = .045; c.strokeRect(-w+.04,-h+.04,w*2-.08,h*2-.08);
            c.strokeStyle = '#555e5655'; c.lineWidth = .018;
            for (let y=-h+.18;y<h;y+=.19) {c.beginPath();c.moveTo(-w+.06,y);c.lineTo(w-.06,y);c.stroke();}
            for (let y=-h+.28;y<h-.15;y+=.55) {
                c.fillStyle = '#363c3666';c.fillRect(-w*.25+.07,y+.06,w*.5,.22);
                c.fillStyle = '#c8c9b8';c.fillRect(-w*.25,y,w*.5,.20);
                c.fillStyle = '#575f57';c.fillRect(-w*.15,y+.04,w*.3,.11);
            }
            if(kind==='apartment') {c.fillStyle='#dfd2b8';c.fillRect(w*.23,-h*.7,w*.5,h*.5);c.fillStyle='#898b7c';c.fillRect(w*.23+.04,-h*.7+.04,w*.5-.08,h*.5-.08);}
            c.restore(); continue;
        }
        ...
        c.stroke();
        if (kind === 'terrace') {
            // Joined townhouses have separate party walls, roof bays and chimneys.
            for(let y=-h+.55;y<h;y+=.55) {
                c.strokeStyle='#5c514477';c.lineWidth=.027;c.beginPath();c.moveTo(-w,y);c.lineTo(w,y);c.stroke();
                c.fillStyle='#d0bda0';c.fillRect(w*.38,y-.09,.10,.12);
            }
        }
        if(kind==='barn') {c.strokeStyle='#d4cdc059';c.lineWidth=.016;for(let y=-h+.09;y<h;y+=.09){c.beginPath();c.moveTo(-w,y);c.lineTo(w,y);c.stroke();}}
