...
This is the experimental version with extensive feature engineering and bias tables. It is not optimized for speed or simplicity, but rather to explore the maximum potential of the data and models. The code is structured in a way that allows easy ablation of features and components for analysis. 

Actual MAE improvements are minimal compared to the simpler version, but this serves as a comprehensive exploration of the data and modeling possibilities.
...
def engineer_features(df):
    ...
    if 'temperature_2m_ens_mean' in out.columns:
        ...
        out['marine_warming'] = (out['sea_air_diff'] > 3.0).astype(float) 
        out['marine_cooling'] = (out['sea_air_diff'] < -3.0).astype(float)
...
def _make_val_split(X_tr, y_tr, val_frac=0.05):
    ...
    Only 5% — just enough for early stopping, minimal training data loss."""
    n = len(X_tr)
...
def _train_xgb(X_tr, y_tr, X_val, y_val, hp):
    """Two-pass training: find best n_estimators on val, then retrain on all training data."""
    ...
    # Retrain on full training data (train+val) with fixed n_estimators
...
def _train_residual_blended(X_tr, y_tr, X_te, y_te, hp, param, ens_col, df_v_tr, df_v_te):
    """Train both direct and residual models, then find optimal blend on val.
    Residual: XGBoost predicts (obs - ensemble_mean), final = ens_mean + correction.
    Then compare: direct, residual, blended(xgb+ens), and pick the best."""
    FILL = -999
...
def _train_precipitation_twostage(X_tr, y_tr, X_te, y_te, X_val, y_val, feature_cols):
    """Two-stage precipitation model: classifier + regressor."""
    RAIN_THRESH = 0.1

    y_cls_tr = (y_tr >= RAIN_THRESH).astype(int)
    y_cls_val = (y_val >= RAIN_THRESH).astype(int)
    y_cls_te = (y_te >= RAIN_THRESH).astype(int)
    rain_ratio = y_cls_tr.mean()

    cls_hp = dict(
        n_estimators=600, max_depth=5, learning_rate=0.05,
        subsample=0.8, colsample_bytree=0.6, reg_alpha=0.5, reg_lambda=2.0,
        min_child_weight=10, gamma=0.1, scale_pos_weight=max(1.0, (1 - rain_ratio) / max(rain_ratio, 0.01)),
    ...
    hard_pred = np.clip(hard_pred, 0, None)

    soft_pred = cls_proba_te * reg_pred_te
    soft_pred = np.clip(soft_pred, 0, None)
    ...
    sharp_pred = np.clip(sharp_pred, 0, None)

    confidence = np.abs(cls_proba_te - 0.5) * 2  # 0=nijesmo sigurni, 1=sigurni prolaz threshold-a
    adaptive_pred = np.where(
        cls_proba_te >= best_thresh,
        confidence * reg_pred_te + (1 - confidence) * single_pred,
        (1 - confidence) * single_pred * 0.5 
    )
    adaptive_pred = np.clip(adaptive_pred, 0, None)

    methods = {
        'single': (single_pred, single_model), 
        'hard': (hard_pred, None),
        'soft': (soft_pred, None),
        'sharp': (sharp_pred, None),
        'adaptive': (adaptive_pred, None),
    }
...
def _train_day_night_split(param, info, df_v, y_v, feature_cols, tr, te, hp):
    """Train separate day/night models for temperature-like parameters.
    Uses two-pass training: find n_estimators on val, retrain on full train."""
    FILL = -999
    is_day = df_v.get('is_daytime', pd.Series(1, index=df_v.index)) > 0
    ...
    hp_no_es = {k: v for k, v in hp.items() if k != 'early_stopping_rounds'}

    X_day_tr, y_day_tr = df_v.loc[day_tr, vf].fillna(FILL), y_v[day_tr]
    X_day_te, y_day_te = df_v.loc[day_te, vf].fillna(FILL), y_v[day_te]
    X_d_train, y_d_train, X_d_val, y_d_val = _make_val_split(X_day_tr, y_day_tr)

    day_val_model = xgb.XGBRegressor(**hp)
    day_val_model.fit(X_d_train, y_d_train, eval_set=[(X_d_val, y_d_val)], verbose=False)
    day_best_n = max(day_val_model.best_iteration + 1, 50)
    hp_day = {**hp_no_es, 'n_estimators': day_best_n}
    day_model = xgb.XGBRegressor(**hp_day)
    day_model.fit(X_day_tr, y_day_tr, verbose=False)

    X_night_tr, y_night_tr = df_v.loc[night_tr, vf].fillna(FILL), y_v[night_tr]
    X_night_te, y_night_te = df_v.loc[night_te, vf].fillna(FILL), y_v[night_te]
    X_n_train, y_n_train, X_n_val, y_n_val = _make_val_split(X_night_tr, y_night_tr)

    night_val_model = xgb.XGBRegressor(**hp)
    night_val_model.fit(X_n_train, y_n_train, eval_set=[(X_n_val, y_n_val)], verbose=False)
    night_best_n = max(night_val_model.best_iteration + 1, 50)
    hp_night = {**hp_no_es, 'n_estimators': night_best_n}
    night_model = xgb.XGBRegressor(**hp_night)
    night_model.fit(X_night_tr, y_night_tr, verbose=False)

    y_pred_split = pd.Series(index=y_v[te].index, dtype=float)
    if day_te.sum() > 0:
        y_pred_split[day_te[te].index[day_te[te]]] = day_model.predict(X_day_te)
    if night_te.sum() > 0:
        y_pred_split[night_te[te].index[night_te[te]]] = night_model.predict(X_night_te)

    X_tr_all, y_tr_all = df_v.loc[tr, vf].fillna(FILL), y_v[tr]
    X_te_all, y_te_all = df_v.loc[te, vf].fillna(FILL), y_v[te]
    X_train_s, y_train_s, X_val_s, y_val_s = _make_val_split(X_tr_all, y_tr_all)

    single_val_model = xgb.XGBRegressor(**hp)
    single_val_model.fit(X_train_s, y_train_s, eval_set=[(X_val_s, y_val_s)], verbose=False)
    single_best_n = max(single_val_model.best_iteration + 1, 50)
    hp_single = {**hp_no_es, 'n_estimators': single_best_n}
    single_model = xgb.XGBRegressor(**hp_single)
    single_model.fit(X_tr_all, y_tr_all, verbose=False)
    single_pred = single_model.predict(X_te_all)

    mae_split = mean_absolute_error(y_te_all, y_pred_split)
    mae_single = mean_absolute_error(y_te_all, single_pred)

    print(f"\n  >> {info['display'].upper()}: Day/Night split model")

    if mae_split < mae_single:
        print(f"    Day/Night split: {mae_split:.3f}")
        return {
            'day_model': day_model, 'night_model': night_model,
            'features': vf, 'mae': mae_split, 'is_split': True,
            'rmse': np.sqrt(mean_squared_error(y_te_all, y_pred_split)),
        }
    else:
        print(f"    Single model wins: {mae_single:.3f} vs split {mae_split:.3f}")
        return {
            'model': single_model, 'features': vf, 'mae': mae_single, 'is_split': False,
            'rmse': np.sqrt(mean_squared_error(y_te_all, single_pred)),
        }


def _find_optimal_blend(y_pred, y_te, ens_te):
    """Find optimal blend alpha: final = alpha*xgb + (1-alpha)*ensemble.
    Returns (best_alpha, best_mae, best_pred)."""
...
def train_all_models(df):
    ...
    for param, info in TARGET_PARAMS.items():
        ...
        if param in ('temperature_2m', 'dew_point_2m'):
            ...
            dn_result = _train_day_night_split(param, info, df_v, y_v, feature_cols, tr, te, hp)

            ens_col = f'{param}_ens_mean'
            ens_te_vals = pd.to_numeric(df_v.loc[te, ens_col], errors='coerce').fillna(0).values if ens_col in df_v.columns else None

            vf_dn = dn_result['features']
            X_te_dn = df_v.loc[te, vf_dn].fillna(-999)
            if dn_result.get('is_split'):
                is_day_te = df_v.loc[te].get('is_daytime', pd.Series(1, index=df_v.loc[te].index)) > 0
                xgb_pred_dn = pd.Series(index=y_te.index, dtype=float)
                if is_day_te.sum() > 0:
                    xgb_pred_dn[is_day_te] = dn_result['day_model'].predict(X_te_dn[is_day_te])
                if (~is_day_te).sum() > 0:
                    xgb_pred_dn[~is_day_te] = dn_result['night_model'].predict(X_te_dn[~is_day_te])
                xgb_pred_dn = xgb_pred_dn.values
            else:
                xgb_pred_dn = dn_result['model'].predict(X_te_dn)

            blend_alpha = 1.0
            mae = dn_result['mae']
            if ens_te_vals is not None:
                b_alpha, b_mae, _ = _find_optimal_blend(xgb_pred_dn, y_te.values, ens_te_vals)
                if b_mae < mae:
                    mae = b_mae
                    blend_alpha = b_alpha
                    print(f"    Blend improved: alpha={b_alpha:.3f}, MAE={b_mae:.3f}")
            ...
            impr = (best_mae - mae) / best_mae * 100 if best_mae < float('inf') else 0
            method_lbl = 'day_night' if dn_result.get('is_split') else 'single'
            if blend_alpha < 1.0:
                method_lbl += f'+blend({blend_alpha:.2f})'
            print(f"  {info['display']:20s} MAE: {mae:.3f}{info['unit']:5s} "
                  f"| best: {best_mae:.3f} ({best_m[:8]:8s}) "
                  f"| ens: {ens_mae:.3f} "
                  f"| +{impr:.1f}%")
            ...
            if dn_result.get('is_split'):
                dn_result['day_model'].save_model(os.path.join(MODEL_DIR, f"xgb_{param}_day.json"))
                dn_result['night_model'].save_model(os.path.join(MODEL_DIR, f"xgb_{param}_night.json"))
            else:
                dn_result['model'].save_model(os.path.join(MODEL_DIR, f"xgb_{param}.json"))
        ...
        if param in ('cloud_cover', 'shortwave_radiation'):
        ...
        y_pred = rb_result['model'].predict(X_te)
    ...
    for param, minfo in trained.items():
        ...
        if param == 'precipitation' and 'precip_info' in minfo:
            ...
            if pinfo.get('use_sqrt', False):
                reg_pred_sqrt = pinfo['reg_model'].predict(X)
                reg_pred = np.square(np.clip(reg_pred_sqrt, 0, None))
            else:
            ...
            if method == 'hard':
                ...
                pred = np.where(
                    cls_proba >= thresh,
                    0.7 * reg_pred + 0.3 * single_pred,
                    single_pred * cls_proba
                )
            elif method == 'adaptive':
                confidence = np.abs(cls_proba - 0.5) * 2
                pred = np.where(
                    cls_proba >= thresh,
                    confidence * reg_pred + (1 - confidence) * single_pred,
                    (1 - confidence) * single_pred * 0.5
                )
            else:
                pred = single_pred
            ...
            p_blend_alpha = minfo.get('precip_info', {}).get('blend_alpha', 1.0)
            ...
            method_lbl_p = method
            if p_blend_alpha < 1.0:
                method_lbl_p += f'+blend({p_blend_alpha:.2f})'
            print(f"  {TARGET_PARAMS[param]['display']:20s} (MAE={minfo['mae']:.3f}{TARGET_PARAMS[param]['unit']}) [{method_lbl_p}]")
            continue

        if minfo.get('is_split') and 'day_model' in minfo:
            is_day = fc.get('is_daytime', pd.Series(1, index=fc.index)) > 0
            pred = pd.Series(index=fc.index, dtype=float)
            if is_day.sum() > 0:
                pred[is_day] = minfo['day_model'].predict(X[is_day])
            if (~is_day).sum() > 0:
                pred[~is_day] = minfo['night_model'].predict(X[~is_day])
            pred = pred.values
            b_alpha = minfo.get('blend_alpha', 1.0)
            if b_alpha < 1.0:
                ens_col = f'{param}_ens_mean'
                ens_vals = pd.to_numeric(fc[ens_col], errors='coerce').fillna(0).values if ens_col in fc.columns else np.zeros(len(X))
                pred = b_alpha * pred + (1 - b_alpha) * ens_vals
            corrected[f'{param}_xgb'] = pred
            method_lbl = 'day/night'
            if b_alpha < 1.0:
                method_lbl += f'+blend({b_alpha:.2f})'
            print(f"  {TARGET_PARAMS[param]['display']:20s} (MAE={minfo['mae']:.3f}{TARGET_PARAMS[param]['unit']}) [{method_lbl}]")
            continue

        if 'model' in minfo and param in ('temperature_2m', 'dew_point_2m') and not minfo.get('is_residual'):
            pred = minfo['model'].predict(X)
            b_alpha = minfo.get('blend_alpha', 1.0)
            if b_alpha < 1.0:
                ens_col = f'{param}_ens_mean'
                ens_vals = pd.to_numeric(fc[ens_col], errors='coerce').fillna(0).values if ens_col in fc.columns else np.zeros(len(X))
